...
def run_model(threshold=0.5, signal = None):
    ...
    checkpoint_load = torch.load('model_24khz.pkl', map_location=torch.device('cpu')) # put the actualy model path in the first variable
...
example_signal , fs = torchaudio.load('Fishing Boats_Full Cabin_03.09.23_133017_20secCPA.wav', backend="soundfile")  

# create your own signal for testing and send it to the function like this:

result = run_model(0.5, example_signal)
