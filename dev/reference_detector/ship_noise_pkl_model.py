...
from reference_detector.mobilenet1d import MobileNetV2
import soundfile as sf
# import scipy.io as scio
import os

def run_model(threshold=0.5, signal = None):
    seed = 1
    ...
    checkpoint_load = torch.load('../dev/reference_detector/model_24khz.pkl', map_location=torch.device('cpu')) # put the actualy model path in the first variable
    ...
    with torch.no_grad():
        ...
        # print(f"Prediction: {'Ship' if class_label == 1 else 'No Ship'} | Probability of Ship: {ship_prob:.4f} | Probability of No Ship: {no_ship_prob:.4f}")
        return {
            "label": class_label,
            "probability_no-ship": no_ship_prob,
            "probability_ship": ship_prob
        }


if __name__ == "__main__":
    wav_file = 'dev/external/version_for_itamar/Fishing Boats_Full Cabin_03.09.23_133017_20secCPA.wav'
    signal, fs = sf.read(wav_file)
    signal = torch.from_numpy(signal)

    # create your own signal for testing and send it to the function like this:

    result = run_model(0.5, signal)
